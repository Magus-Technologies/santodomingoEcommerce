...
}elseif ($tipo == 'pag-search2PD') {

    $prod = $_POST['pal'];

    $resp = $productoDao->getListaProd4SearchPag(
        $prod,
        $_POST['forma'],
        $_POST['cntPH'],
        $_POST['hojaActual'],
...
} elseif ($tipo == 'pag-search2PDV') {

    $prod = $_POST['pal'];

    $resp = $productoDao->getListaProd5SearchPag(
        $prod,
        $_POST['forma'],
        $_POST['cntPH'],
        $_POST['hojaActual'],
...
WHERE prod_id = '$prod';";
    ...
    $estado = $_POST['estado'];
    $OsubCat = $_POST['OsubCat'];
    $marcaNew = $_POST['marcaNew'];	
   ...
   if ($OsubCat !== '') {
	$sqlfil = "SELECT id_seleccion FROM grupo_seleccion WHERE codi_categoria='{$_POST['cod_cat']}'";
       $result3 = $productoDao->exeSQL($sqlfil);
	if ($result3->num_rows > 0) {
            foreach ($result3 as $nuevo) {
                $catcodi = $nuevo['id_seleccion'];
            }
        }

        $sql = "INSERT INTO sub_categoria SET id_catego='{$catcodi}',nombre='{$_POST['OsubCat']}'";
        $productoDao->exeSQL($sql);

	 $sqlfi = "SELECT sub_id FROM sub_categoria WHERE id_catego='{$catcodi}' AND nombre='{$_POST['OsubCat']}'";
         $result4 = $productoDao->exeSQL($sqlfi);
	  if ($result4->num_rows > 0) {
            foreach ($result4 as $nuev) {
                $subCat = $nuev['sub_id'];
            }
        }
    }	
    ...
    $sqlm = "UPDATE producto SET marca='$marcaNew' WHERE prod_id='$idPro'";
	$resultm = $productoDao->exeSQL($sqlm);
    ...
    foreach ($res as $row) {
        ...
        $conRay = $productosApi->getDataProd($row['prod_cod'], $filtro);
