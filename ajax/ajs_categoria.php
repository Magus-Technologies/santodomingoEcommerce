...
FROM grupo_seleccion ";
    ...
    $imagen = $_POST['imagen'];
    $nombre = $_POST['nombre'] ?? '';
    $sql = "UPDATE grupo_seleccion SET imagen = '$imagen', nombre_cate = '$nombre' WHERE id_seleccion = '{$_POST['cate']}'";
    if ($conexion->query($sql)) {
        $respuesta['res'] = true;
    }
} elseif ($tipo == 'toggle_estado') {
    $sql = "UPDATE grupo_seleccion SET estado = IF(estado='1','0','1') WHERE id_seleccion = '{$_POST['id']}'";
    if ($conexion->query($sql)) {
        $respuesta['res'] = true;
    }
} elseif ($tipo == 'del') {
    $sql = "DELETE FROM grupo_seleccion WHERE id_seleccion = '{$_POST['id']}'";
    if ($conexion->query($sql)) {
        $respuesta['res'] = true;
    }
} elseif ($tipo == 'sub_c') {
...
WHERE grupo_seleccion.codi_categoria ='{$_POST['cat']}' AND nombre !=''";
