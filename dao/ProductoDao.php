...
class ProductoDao extends Producto
{
  ...
  producto.prod_cod,
  producto.sub_cat FROM producto 
  LEFT JOIN stocks ON (stocks.cod_prod = producto.prod_cod AND stocks.`cod_suc` = 1)
	WHERE   producto.categoria ='$categ' AND producto.estado = '1' AND stock_prod>'0' AND stocks.stock_act>0";
        return $this->conexion->query($sql);
    }
...
WHERE 
   producto.categoria = '$cate' ";
    ...
    public function getData()
    {
	   ...
	   //buscar stock activo
	    $sqlstock = "SELECT SUM(stock_act) AS 'stock' FROM stocks WHERE (cod_alma='109' OR cod_alma='105' OR cod_alma ='101' OR cod_alma = '301') 
	   		AND cod_prod =  '{$row['prod_cod']}'";
	      $rsul = $this->conexion->query($sqlstock);
...
GROUP BY prodod.nombre_marca";
    ...
    public function getListaProd3SearchPag($pals, $tipo, $cnthp, $actual, $minUSD, $maxISD, $filtro)
    {
        $actual = $actual * $cnthp;
	...
	$opalabra = explode(" ", trim($pals));
       $cpal = str_word_count($pals);
       $listaPal = "";
       foreach ($opalabra as $fil) {
	   $listaPal = $listaPal." OR producto.nombre LIKE '%$fil%'";	
       }
	$listaPal = $listaPal. " ) ";
...
FROM
  (SELECT 
    prodod.*, (CASE WHEN '$filtro'='' THEN precios.precio_venta 
  ...
  FROM
    ...
    FROM
      producto 
    WHERE producto.estado = 1 AND (producto.nombre LIKE '%$pals%' $listaPal $listaTempMarc ORDER BY producto.prod_id DESC  $contLimits) prodod 
...
";*/
  ...
  public function getListaProd5SearchPag($pals, $tipo, $cnthp, $actual, $minUSD, $maxISD, $filtro)
    {
        $actual = $actual * $cnthp;
        $limitador = ' limit ' . $actual . ',' . $cnthp;
        if ($tipo == 'last') {
...
//            $contLimits = 'LIMIT 400';
		...
		$like = " AND nombre LIKE '%$pals%'";
	  }

	 $catf ="";
	 if ($filtro !='') {
		$catf = " AND categoria ='$filtro'";
  	 }
...
FROM
  (SELECT 
    prodod.*, (CASE WHEN precios.precio_tres <=1 THEN (precios.precio_venta-(precios.precio_venta*0.03)) 
		  ELSE precios.precio_tres END) AS precio
  FROM
    (SELECT 
      producto.content1,
      producto.content2,
      ...
      producto.estado ,
	    0 as inistock,
	    producto.categoria
      FROM producto  WHERE producto.estado = 1)  prodod 
      JOIN precios 
      ON precios.cod_prod = prodod.prod_cod 
      AND precios.cod_suc = 1 
  GROUP BY prodod.prod_cod) temp 
  JOIN `stocks`  ON ( `stocks`.`cod_prod` = `temp`.`prod_cod`  AND (cod_alma='109' OR cod_alma='105' OR cod_alma ='101' OR cod_alma = '301' )) 
    AND stocks.stock_act > 0".$like.$catf." GROUP BY temp.prod_cod $limitador";
  ...
  public function getListaProd4SearchPag($pals, $tipo, $cnthp, $actual, $minUSD, $maxISD, $filtro)
    {
        $actual = $actual * $cnthp;
        $limitador = ' limit ' . $actual . ',' . $cnthp;
        if ($tipo == 'last') {
...
FROM
  (SELECT 
    prodod.*, (CASE WHEN precios.precio_mayor  <=1 THEN (precios.precio_venta-(precios.precio_venta*0.05)) 
		  ELSE precios.precio_mayor END) AS precio
  FROM
    (SELECT 
      producto.content1,
      producto.content2,
...
FROM
  producto 
  INNER JOIN precios p on p.cod_prod = producto.prod_cod
...
FROM
  producto INNER JOIN marcra_productos AS marca ON producto.marca = marca.cod_marca where  producto.tipo_pro = '2' ";
    ...
    public function getListRandonRelacionada($cnt)
    {
        ...
        */
        $sql = "SELECT tableTemp.* FROM (SELECT  pf.imagen_url,prod2.stock_act,prob1.prod_cod,prob1.prod_id,propre.precio_cuatro,
SUM(prod2.stock_act) AS suma_stock,prob1.nombre,op.precio_oferta, ROUND(RAND() * prob1.prod_id) 'rand_ind' FROM producto AS prob1 
	...
	producto.*,
	e.id_exclu AS id,
	p.precio_cinco AS precio_ofertaa 
FROM
	producto
	INNER JOIN precios p on p.cod_prod = producto.prod_cod
	INNER JOIN productos_exclusivos e on e.prod_id = producto.prod_id

	LEFT JOIN ofertas_productos AS op ON producto.prod_id = op.producto_id 
...
WHERE producto.estado = '1' and producto.estado_prod='1'  ORDER BY producto.prod_id DESC LIMIT $num";
...
WHERE NOW() <= ofer.fecha_termino order by rand()";
    ...
    public function getProductosExclusivos($tab_name)
    {
        $tab_name = $this->conexion->real_escape_string($tab_name);
        $sql = "SELECT pe.*, p.prod_id, p.prod_cod, p.nombre, p.prod_prec_ven as precio, p.prod_tipo as tipo_pro, p.prod_stock as stock, p.prod_id as prod_id
                FROM productos_exclusivos pe
                JOIN producto p ON pe.producto_id = p.prod_id
                WHERE pe.tab_name = '$tab_name' AND pe.estado = '1'
                ORDER BY pe.orden ASC";
        
        $res = $this->conexion->query($sql);
        $data = [];
        while ($row = $res->fetch_assoc()) {
            $row['stock_prod'] = 0; // Fallback for storefront checks
            $row['precio_ofertaa'] = null; // Fallback or fetch from elsewhere if needed

            // Buscar imágenes del producto por defecto
            $sqlImg = "SELECT imagen_url FROM producto_foto WHERE prod_id = {$row['prod_id']} ORDER BY orden ASC LIMIT 2";
            $resImg = $this->conexion->query($sqlImg);
            $imgs = [];
            while($rI = $resImg->fetch_assoc()){
                $imgs[] = $rI['imagen_url'];
            }
            
            $row['imagen1'] = $imgs[0] ?? 'sinimagen_mtr_20sba.jpg';
            $row['imagen2'] = $imgs[1] ?? $row['imagen1'];

            // Si tiene imagen propia (exclusiva), sobreescribimos la principal
            if (!empty($row['imagen'])) {
                $row['imagen1'] = '../../facturacion_santoDomingo/storage/' . $row['imagen'];
                // Para la de hover, si es exclusiva, podríamos dejar la misma o la del producto original
            }
