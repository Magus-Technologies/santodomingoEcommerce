const CARRITO = new Vue({
    ...
    methods: {
        ...
        eliminarProdCarr(index) {
            ...
            // Validar que tenga propiedad cantidad
            if (!('cantidad' in prod)) {
                console.error('El producto no tiene propiedad cantidad:', prod);
                return;
            }
        ...
        espe_prod_carr(prod) {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (stock > 0) {
                        if (temp_ind != -1) {
                            if (CARRITO._data.listaCarrito[temp_ind].cantidad + 1 < stock - 3 && stock > 0) {
                            ...
                            if (resp.precio_oferta !== null && resp.precio_oferta !== "0.00") {
        ...
        getDataCarrito() {
            if (!IS_LOGIN) {
                ...
                if (car) {
                    ...
                    this.listaCarrito = this.listaCarrito.filter(itemm => itemm.stock ? true : false).filter(itemm => itemm.cantidad <= itemm.stock)
        ...
        getUsuarioCarrito() {
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    const lisTemp = CARRITO._data.listaCarrito.filter(itemm => itemm.stock ? true : false).filter(itemm => itemm.cantidad <= itemm.stock)
